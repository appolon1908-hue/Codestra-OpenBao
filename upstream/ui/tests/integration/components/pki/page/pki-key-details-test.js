...
module('Integration | Component | pki key details page', function (hooks) {
  ...
  hooks.beforeEach(function () {
    ...
    this.store.pushPayload('pki/key', {
      modelName: 'pki/key',
      key_id: '<CODESTRA_GITLEAKS_FIXTURE_INVALID>',
      key_type: 'ec',
      key_name: 'test-key',
    });
    this.model = this.store.peekRecord('pki/key', '<CODESTRA_GITLEAKS_FIXTURE_INVALID>');
  });
  ...
  test('it renders the page component and deletes a key', async function (assert) {
    ...
    assert.dom(SELECTORS.keyIdValue).hasText(' <CODESTRA_GITLEAKS_FIXTURE_INVALID>', 'key id renders');
