...
**_Users_** authenticating with JWT or OIDC tokens will have their claims validated through the CEL indicated role. If successful, they receive a token with policies and other attributes.
...
Dynamic policy assignment can be implemented using templating for [ACLs](/docs/api/auth/jwt/#acl-policy-templating-examples), but the expressiveness of CEL is much greater.

Some form CEL or regex evaluation could be added to existing role instead of creating a new type, as with [glob matching](/docs/api/auth/jwt/#createupdate-role) for `bound_claims`.
