...
[response-wrapping](/docs/concepts/response-wrapping) workflow because
...
Requesting user can now retrieve the response if the number of authorizations is equal to the approvers attribute:
...
Involving a second actor in securing a path creates some complexity and impedance for users.
