...
- `allowed_client_ids` `([]string: <optional>)` <a name="allowed_client_ids"></a> – The client IDs that are permitted to use the provider. If empty, no clients are allowed. If `"*"` is provided, all clients are allowed.
...
- `template` `(string: <optional>)` - The [JSON template](../../../concepts/oidc-provider.mdx#scopes)
...
- `key` `(string: "default")` – A reference to a [named key](tokens.mdx#create-a-named-key)
  ...
  after creation. If not supplied, defaults to the built-in [default key](../../../concepts/oidc-provider.mdx#keys).
...
- `assignments` `([]string: <optional>)` – A list of assignment resources associated with
  ...
  [allow_all](../../../concepts/oidc-provider.mdx#assignments) assignment.
...
- `id_token_ttl` `(int or duration: "24h")` – The time-to-live for ID tokens obtained by the client.
  Accepts [duration format strings](../../../concepts/duration-format.mdx). The value should be less than the `verification_ttl`
  on the key.

- `access_token_ttl` `(int or duration: "24h")` – The time-to-live for access tokens obtained by the client.
  Accepts [duration format strings](../../../concepts/duration-format.mdx).
...
- `entity_ids` `([]string: <optional>)` - A list of OpenBao [entity](../../../concepts/identity.mdx#entities-and-aliases) IDs.

- `group_ids` `([]string: <optional>)` – A list of OpenBao [group](../../../concepts/identity.mdx#identity-groups) IDs.
...
```json
{
  "access_token": "OPENBAO_BATCH_TOKEN_FIXTURE_INVALID",
  "expires_in": 3600,
  "id_token": "<CODESTRA_GITLEAKS_FIXTURE_INVALID>",
  "token_type": "Bearer"
}
```
