...
the [PKI documentation](../../secrets/pki/index.mdx).
...
- [Issuing CEL Certificates](#issuing-cel-certificates)
  - [List CEL Roles](#list-cel-roles)
  - [Read CEL Role](#read-cel-role)
  - [Generate certificate and key against CEL role](#generate-certificate-and-key-against-cel-role)
  - [Sign certificate against CEL role](#sign-certificate-against-cel-role)
...
- [Managing CEL Authority Information](#managing-cel-authority-information)
  - [List CEL Roles](#list-cel-roles)
  - [Create/Update CEL Role](#create-update-cel-role)
  - [Read CEL Role](#read-cel-role)
  - [Delete CEL Role](#delete-cel-role)
...
#### Parameters
 ...
 - `dns_resolver` `(string: "")` <a name="dns_resolver"></a> - An optional overriding DNS resolver to
 ...
 - `eab_policy` `(string: "not-required")` <a name="eab_policy"></a> - Specified policy to enforce
...
```json
{
  ...
  "data": {
    ...
    "private_key": "PRIVATE_KEY_TEST_FIXTURE_REMOVED\n",
...
- `permitted_dns_domains` `(string: "")` <a name="permitted_dns_domains"></a> - A comma separated string (or, string
...
```json
{
  "pem_bundle": "PRIVATE_KEY_TEST_FIXTURE_HEADER_REMOVED\n...\n-----END CERTIFICATE-----"
}
```
...
- `leaf_not_after_behavior` `(string: "err")` <a name="leaf_not_after_behavior"></a> - Behavior of a leaf's
...
- `enable_aia_url_templating` `(bool: false)` <a name="enable_aia_url_templating"></a> - Specifies that the above AIA
...
- `ttl` `(string: "")` - Specifies the Time To Live value to be used for the
  ...
  [duration string](../../concepts/duration-format.mdx). The value specified is
...
- `max_ttl` `(string: "")` - Specifies the maximum Time To Live provided as a
  [duration string](../../concepts/duration-format.mdx). If not set, defaults to
...
- `allowed_domains` `(list: [])` <a name="allowed_domains"></a> - Specifies the domains this role is allowed
...
- `allowed_domains_template` `(bool: false)` - When set, `allowed_domains`
  may contain templates, as with [ACL Path Templating](../../concepts/policies.mdx).
...
- `allow_globs_in_identity_templates` `(bool: false)` - If set and templating is
...
- `default_follows_latest_issuer` `(bool: false)` - Specifies whether a
  ...
  associated with [multi-issuer rotation](../../secrets/pki/rotation-primitives.mdx).
...
- `safety_buffer` `(string: "")` <a name="safety_buffer"></a> - Specifies a duration using [duration format strings](../../concepts/duration-format.mdx)
...
- `revoked_safety_buffer` `(string: "")` - Specifies a duration using
  [duration format strings](../../concepts/duration-format.mdx) used as a
...
- `revocation_queue_safety_buffer` `(string: "")` - Specifies a duration
 ...
 - `tidy_acme` `(bool: false)` <a name="tidy_acme"></a> - Set to true to tidy stale ACME accounts,
 ...
 - `acme_account_safety_buffer` `(string: "720h")` <a name="acme_account_safety_buffer"></a> - The amount of time that
...
## Managing CEL authority information
...
prgraom created by the operator; limited to no validation is performed by
...
- `cel_program` `(CelProgram: <required>)` - Specifies the CEL Variable list and main
  CEL expression. Refer to [Quick Start - CEL in PKI](../../secrets/pki/quick-start-cel.mdx) for more
...
See [PKI Cluster Scalability](../../secrets/pki/considerations.mdx#cluster-scalability) in the considerations page.
