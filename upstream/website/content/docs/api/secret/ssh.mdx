...
the [SSH documentation](../../secrets/ssh/index.mdx).
...
- `ttl` `(string: "")` – Specifies the Time To Live value provided as a
  [duration string](../../concepts/duration-format.mdx). If not set, uses the
...
- `not_before_duration` `(duration: "30s")` – Specifies the duration by which to
  backdate the `ValidAfter` property. Uses [duration format strings](../../concepts/duration-format.mdx).
...
- `allow_commas_in_identity_templates` `(bool: false)` - If set and templating
...
```json
{
  ...
  "data": {
    "ip": "127.0.0.1",
    "key": "<CODESTRA_GITLEAKS_FIXTURE_INVALID>",
    "key_type": "otp",
    "port": 22,
    "username": "rajanadar"
  },
