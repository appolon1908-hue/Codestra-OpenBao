...
see the [OpenBao Userpass method documentation](../../auth/userpass.mdx).
...
- `password_hash` `(string: <required>)` - Pre-hashed bcrypt password for this user. Mutually exclusive with `password`.
...
```json
{
  ...
  "warnings": null,
  "auth": {
    "client_token": "OPENBAO_TOKEN_FIXTURE_INVALID",
