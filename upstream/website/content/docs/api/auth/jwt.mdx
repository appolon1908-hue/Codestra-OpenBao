...
[OpenBao JWT/OIDC method documentation](../../auth/jwt/index.mdx).
...
- `user_claim` `(string: <required>)` <a name="user_claim"></a> - The claim to use to uniquely identify
...
- `user_claim_json_pointer` `(bool: false)` - Specifies if the `user_claim` value uses
  [JSON pointer](../../auth/jwt/index.mdx#claim-specifications-and-json-pointer) syntax for
...
- `bound_claims` `(map: <optional>)` - If set, a map of claims (keys) to match against respective claim values (values).
  ...
  claim values is configured with `bound_claims_type`. Keys support [JSON pointer](../../auth/jwt/index.mdx#claim-specifications-and-json-pointer)
  syntax for referencing claims.
- `bound_claims_type` `(string: "string")`<a name="bound_claims_type"></a> - Configures the interpretation of the bound_claims values.
  ...
  characters.
- `groups_claim` `(string: <optional>)` <a name="groups_claim"></a> - The claim to use to uniquely identify
  the set of groups to which the user belongs; this will be used as the names
  for the Identity group aliases created due to a successful login. The claim
  value must be a list of strings. Supports [JSON pointer](../../auth/jwt/index.mdx#claim-specifications-and-json-pointer)
  syntax for referencing claims.
- `claim_mappings` `(map: <optional>)` <a name="claim_mappings"></a> - If set, a map of claims (keys) to be copied to
  specified metadata fields (values). Keys support [JSON pointer](../../auth/jwt/index.mdx#claim-specifications-and-json-pointer)
  syntax for referencing claims but may also be direct claim key names.
  [See claim mapping documentation](../../auth/jwt/index.mdx#claims-as-metadata) for
  more information.
...
- `poll_interval` `(int: <optional>)` - Poll interval in seconds for device
  ...
  device flow, or '5'.
- `oidc_disable_confirmation` `(bool: false)` - If set to `true`, disables the interactive confirmation page that Openbao
  displays during direct callback mode before processing the authorization code. This confirmation step is a security measure
  against token-hijacking attacks (see [RFC 8628 5.4](https://www.rfc-editor.org/rfc/rfc8628#section-5.4)) and is enabled by default.
  **It's recommended to set `oidc_disable_confirmation` to `true` only if your OIDC provider always presents its own confirmation prompt to the user.**
- `verbose_oidc_logging` `(bool: false)` <a name="verbose_oidc_logging"></a> - Log received OIDC tokens and claims when debug-level
...
- `redirect_uri` `(string: <required except in device callback mode>)` - Path to the callback to complete the login. This will be
  ...
  provider. See [Redirect URIs](../../auth/jwt/index.mdx#redirect-uris) for more information.
...
$ curl \
    --header "X-Vault-Token: ..." \
    --request LIST \
    http://127.0.0.1:8200/v1/auth/jwt/cel/roles
```
...
$ curl \
    --header "X-Vault-Token: ..." \
    http://127.0.0.1:8200/v1/auth/jwt/cel/roles/my-role
```
...
```json
{
  "data": {
    ...
    "bound_audiences": ["openbao.example.org"]
  },
  ...
}
```
...
| Method  | Path                        |
| :------ | :-------------------------- |
| `POST`  | `/auth/jwt/cel/roles/:name` |
| `PATCH` | `/auth/jwt/cel/roles/:name` |

:::warning

CEL is an advanced topic for operators. All validation is taken on by the CEL
program created by the operator; **limited to no validation is performed by
OpenBao above and beyond what the CEL program contains**. It is up to the
operator to craft a CEL role which adequately perform validation for the
provided JWT and all of its claims. Some CEL programs may cause high CPU
usage; OpenBao will validate the requesting JWT is signed by a valid issuer
but the CEL program author must take care not to design expensive programs.

All claims on the JWT (including `aud`, `sub`, and any custom claims) should
be validated and constrained before issuing an OpenBao token based on them.

:::

#### Parameters

- `name` `(string: <required>)` - Specifies the name of the CEL role to
  create. This is part of the request URL.
- `cel_program` `(CelProgram: <required>)` - Specifies the CEL Variable list and main
  CEL expression. Refer to [Quick Start - CEL in JWT](../../auth/jwt/index.mdx#cel) for more
  information about how to construct the CEL program correctly.
- `message` `(string: <optional>)` - Static error message if validation
  failed.
- `clock_skew_leeway` `(int or string: <optional>)` - The amount of leeway to
  add to all claims to account for clock skew, in seconds. Defaults to `60`
  seconds if set to `0` and can be disabled if set to `-1`. Accepts an integer
  number of seconds, or a Go duration format string.
- `expiration_leeway` `(int or string: <optional>)` - The amount of leeway to
  add to expiration (`exp`) claims to account for clock skew, in seconds.
  Defaults to `150` seconds if set to `0` and can be disabled if set to `-1`.
  Accepts an integer number of seconds, or a Go duration format string. Only
  applicable with "jwt" roles.
- `not_before_leeway` `(int or string: <optional>)` - The amount of leeway to
  add to not before (`nbf`) claims to account for clock skew, in seconds.
  Defaults to `150` seconds if set to `0` and can be disabled if set to `-1`.
  Accepts an integer number of seconds, or a Go duration format string. Only
  applicable with "jwt" roles.
- `bound_audiences` `(array: <optional>)` - Optional list of `aud` claims to
  match against.

  :::warning

  Unlike in the standard role mechanism, CEL for JWT does not reject tokens
  which do not provide an audience claim when `bound_audiences` is specified.
  Likewise, it does not require the `bound_audiences` parameter to be
  specified and this may be left blank by the operator; if so, the CEL program
  is entirely responsible for validating this field.
...
revoke tokens issued by this role.

| Method   | Path                        |
| :------- | :-------------------------- |
| `DELETE` | `/auth/jwt/cel/roles/:name` |
...
Fetch a token given a JWT. This endpoint takes a signed JSON Web Token (JWT)
and a CEL role name for some entity. It verifies the JWT signature to
authenticate that entity and then authorizes the entity for the given role.

| Method | Path                  |
| :----- | :-------------------- |
| `POST` | `/auth/jwt/cel/login` |

### Parameters

- `role` `(string: <optional>)` - Name of the CEL role against which the login
  is being attempted.
- `jwt` `(string: <required>)` - Signed [JSON Web Token](https://tools.ietf.org/html/rfc7519) (JWT).
...
$ curl \
    --request POST \
    --data @payload.json \
    https://127.0.0.1:8200/v1/auth/jwt/cel/login
```
