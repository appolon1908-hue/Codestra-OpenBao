...
[OpenBao Kubernetes auth method](../../auth/kubernetes.mdx).
...
- `token_reviewer_jwt` `(string: "")` <a name="token_reviewer_jwt"></a> - A service account JWT used to access the TokenReview
...
- `issuer` `(string: "")` <a name="issuer"></a> **Deprecated** Optional JWT issuer. If no issuer is specified, then this plugin will use `kubernetes/serviceaccount` as the default issuer.
See [these instructions](../../auth/kubernetes.mdx#discovering-the-service-account-issuer) for looking up the issuer for a given Kubernetes cluster.
