...
see the [OpenBao Token method documentation](../../auth/token.mdx).
...
- `ttl` `(string: "")` - The TTL period of the token, provided as a
  [duration string](../../concepts/duration-format.mdx). If not provided, the
  token is valid for the [default lease TTL](../../configuration/index.mdx), or
...
```json
{
  ...
  "auth": {
    "client_token": "OPENBAO_LEGACY_TOKEN_FIXTURE_INVALID",
...
- `token` `(string: <required>)` - Token to renew.
...
[max duration](../../configuration/index.mdx#default_max_request_duration)
