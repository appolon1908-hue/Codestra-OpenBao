...
[OpenBao Kerberos auth method](../../auth/kerberos.mdx).
...
```json
{
  ...
  "keytab": "<CODESTRA_GITLEAKS_FIXTURE_INVALID>",
...
- `decode_pac` `(bool: true)` - Specifies whether to decode the PAC
  (Privilege Attribute Certificate) from the Kerberos ticket.
