...
**Direct callback mode - confirmation page**

When using `callbackmode=direct`, OpenBao shows an intermediate confirmation page to the user after the OIDC
provider redirects back, before the authorization code exchanged. The page shows the requester's IP address
so the user can verify they initiated the login and are not being phished by a crafted authorization link
see [RFC 8628 section 5.4](https://www.rfc-editor.org/rfc/rfc8628#section-5.4). This confirmation step is
enabled by default.

If your OIDC provider always presents its own confirmation prompt to the user, the OpenBao confirmation page
can be disabled by setting `oidc_disable_confirmation = true` on the role. This option should only be used if
the OIDC provider always presents its own confirmation prompt.
...
If the [oidc_response_mode](../../api/auth/jwt.mdx#oidc_response_mode) is set to `form_post`, then
...
- The [`verbose_oidc_logging`](../../api/auth/jwt.mdx#verbose_oidc_logging) role
...
1. Use the `/config` endpoint to configure OpenBao. To support JWT roles, either local keys, a JWKS URL, or an OIDC
   ...
   list of available configuration options, please see the [API documentation](../../api/auth/jwt.mdx).
...
Bound claims can optionally be configured with globs. See the [API documentation](../../api/auth/jwt.mdx#bound_claims_type) for more details.
...
[API docs](../../api/auth/jwt.mdx) for more details.
