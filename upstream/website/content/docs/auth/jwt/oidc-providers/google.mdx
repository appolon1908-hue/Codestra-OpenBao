...
used to copy claims data into resulting auth token and alias metadata via [claim_mappings](../../../api/auth/jwt.mdx#claim_mappings).
...
- `user_custom_schemas` `(string: <optional>)` - Comma-separated list of Google Workspace [custom schemas](https://developers.google.com/admin-sdk/directory/v1/guides/manage-schemas).
  Values set for Google Workspace users using custom schema fields will be fetched and made available as claims that can be used with [claim_mappings](../../../api/auth/jwt.mdx#claim_mappings). Required if `fetch_user_info` is set to true.
...
The [user_claim](../../../api/auth/jwt.mdx#user_claim) value of the role must be set to
