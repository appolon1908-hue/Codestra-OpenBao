...
To connect the AD group with a [OpenBao external groups](../../../concepts/identity.mdx#external-vs-internal-groups),
you will need
...
2. Configure the [OIDC Role](../../../api/auth/jwt.mdx#createupdate-role) with the following:
...
1. In OpenBao, create the [external group](../../../api/secret/identity/group.mdx).
   Record the group ID as you will need it for the group alias.

1. From OpenBao, retrieve the [OIDC accessor ID](../../../api/system/auth.mdx#list-auth-methods)
...
1. In OpenBao, create a [group alias](../../../api/secret/identity/group-alias.mdx)
