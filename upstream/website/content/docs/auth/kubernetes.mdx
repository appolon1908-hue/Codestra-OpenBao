...
1. Use the `/config` endpoint to configure OpenBao to talk to Kubernetes. Use
   ...
   documentation](../api/auth/kubernetes.mdx).
...
[`token_reviewer_jwt`](../api/auth/kubernetes.mdx#token_reviewer_jwt) option.
...
[`issuer`](../api/auth/kubernetes.mdx#issuer) to the same value as
...
[API docs](../api/auth/kubernetes.mdx) for more details.
