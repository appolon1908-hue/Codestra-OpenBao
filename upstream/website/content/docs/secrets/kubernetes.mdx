...
1. Configure the mount point. An empty config is allowed.
   ...
   [API docs](../api/secret/kubernetes.mdx).
...
$ curl -sk $(kubectl config view --minify -o 'jsonpath={.clusters[].cluster.server}')/api/v1/namespaces/test/pods \
    --header "Authorization: Bearer <CODESTRA_BEARER_TOKEN_FIXTURE_INVALID>"
{
  "kind": "PodList",
  "apiVersion": "v1",
  "metadata": {
...
[Kubernetes Secrets Engine API docs](../api/secret/kubernetes.mdx) for more details.
